...
abstract contract Proposal is Test, Script, IProposal {
    ...
    constructor(EnvMock _env) {
        ...
        DO_AFTER_DEPLOY_MOCK = env.envOr("DO_AFTER_DEPLOY_MOCK", true);
        DO_BUILD = env.envOr("DO_BUILD", true);
        DO_SIMULATE = env.envOr("DO_SIMULATE", true);
        DO_VALIDATE = env.envOr("DO_VALIDATE", true);
        DO_PRINT = env.envOr("DO_PRINT", true);
    }
